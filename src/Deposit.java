...
public class Deposit extends JFrame implements ActionListener{
    ...
    Deposit(String PIN){
        ...
        JLabel heading = new JLabel("Enter Amont You Want to Deposit :");
        ...
        try{
            if(e.getSource()==back)
            {
                ...
                setVisible(false);
                    ...
                    {   
                          String textDeposit = amount.getText();
                          if(textDeposit.isEmpty())
                           {
                                JOptionPane.showMessageDialog(null, "Invalid Input.");
                           }else{
                                double a = Double.parseDouble(textDeposit);//if user will enter characters program will not run.
                                connection con = new connection();
                                String query = "insert into BankAccount values('"+PIN+"','"+con.date()+"','Deposit','"+textDeposit+"')"; 
                                con.statement.executeUpdate(query);
                                JOptionPane.showMessageDialog(null, "Amount Rs " +a+ " is successfully added.");
                                new Dashboard(PIN);
                                setVisible(false);
                        }
                    }  
